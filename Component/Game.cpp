...
{
	InputManager::SetContext({ mKeyboard.get(), mMouse.get() });
	...
	//entityManager.CreateEntity("test", StringID("cube"), StringID("wood"),Transform::Create(XMFLOAT3(5, 5, 20)));
}
...
void Game::Update()
{
	...
	if (Input::IsKeyDown(Keyboard::OemTilde) && CurrentTime > delay)
	{
		debugMode = !debugMode;
		CurrentTime = 0.f;
	}
